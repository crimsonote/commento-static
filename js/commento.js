(function(global, document) {
  ...
  function selfLoad(commenter, email) {
    ...
    notificationSettingsButton.innerText = "Notification Settings.通知设置";
    profileEditButton.innerText = "Edit Profile.编辑个人资料";
    logoutButton.innerText = "Logout.登出";
  ...
  function markdownHelpShow(id) {
    ...
    boldLeft.innerHTML = "<b>粗体bold</b>";
    boldRight.innerHTML = "用 <pre>**两个星号**</pre> 包围文本";
    italicsLeft.innerHTML = "<i>斜体italics</i>";
    italicsRight.innerHTML = "用 <pre>*星号*</pre> 包围文本";
    codeLeft.innerHTML = "<pre>代码</pre>";
    codeRight.innerHTML = "用<pre>`反引号`</pre>包围文本";
    strikethroughLeft.innerHTML = "<strike>删除线 strikethrough</strike>";
    strikethroughRight.innerHTML = "用 <pre>~~两个波浪字符~~</pre> 包围文本";
    hyperlinkLeft.innerHTML = "<a href=\"https://example.com\">超链接hyperlink</a>";
    hyperlinkRight.innerHTML = "<pre>[超链接](https://example.com)</pre> 或者仅是一个纯链接";
    quoteLeft.innerHTML = "<blockquote>引用quote</blockquote>";
    quoteRight.innerHTML = "前戳 <pre>&gt;符号</pre>";
  ...
  function textareaCreate(id, edit) {
    ...
    attrSet(textarea, "placeholder", "Add a comment 添加评论");
    attrSet(anonymousCheckbox, "type", "checkbox");
    attrSet(anonymousCheckboxLabel, "for", ID_ANONYMOUS_CHECKBOX + id);

    anonymousCheckboxLabel.innerText = "Comment anonymously 匿名评论";
    if (edit === true) {
      submitButton.innerText = "Save Changes";
    } else {
      submitButton.innerText = "Add Comment 发送评论";
    }
  ...
  global.commentNew = function(id, commenterToken, callback) {
    ...
    post(origin + "/api/comment/new", json, function(resp) {
      ...
      if (resp.state === "unapproved") {
        message = "Your comment is under moderation.您的评论处于审核中。";
  ...
  global.popupRender = function(id) {
    ...
    forgotLink.innerText = "Forgot your password?忘记密码？";
    ...
    attrSet(emailInput, "placeholder", "Email address 电子邮件地址");
